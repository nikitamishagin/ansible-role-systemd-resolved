...
A fork of the [idiv_biodiversity.systemd_resolved](https://github.com/idiv-biodiversity/ansible-role-systemd-resolved)
role (by Christian Krause).
...
| systemd_resolved_update_cache                 | true    | Refresh repository cache before installing the package. Set to false if a previous role already refreshed it to avoid redundant work and speed up runs. |
...
- By default, the role refreshes the package manager repository cache. You can disable this by setting
  systemd_resolved_update_cache: false if another role already performed the refresh to avoid redundant actions and
